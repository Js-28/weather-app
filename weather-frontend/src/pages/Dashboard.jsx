...
//                           {/* {new Date((hour.dt + current.t imezone) * 1000).getUTCHours()}:00 */}
...
export default function Dashboard() {
  ...
  const { current, hourly, loading, error } =
    useSelector((state) => state.weather) || {};

  const { list: cities = [], loading: citiesLoading = false } =
    useSelector((state) => state.cities) || {};

  const { list: notifications = [], city: subscribedCity = null } =
    ...
    const restoreCity = async () => {
      ...
      try {
        const res = await fetch(
          `${import.meta.env.VITE_API_URL}/cities/me`,
          { credentials: "include" }
        );
        const data = await res.json();
        ...
        if (data.city && data.lat && data.lon) {
          dispatch(setSubscribedCity(data.city));
          dispatch(fetchCurrentWeather({ lat: data.lat, lon: data.lon }));
          dispatch(fetchHourlyForecast({ lat: data.lat, lon: data.lon }));
        }
      } catch (err) {
        console.error(err);
      } finally {
  ...
  useEffect(() => {
    ...
    navigator.geolocation.getCurrentPosition(
      async (pos) => {
        ...
        dispatch(
          fetchHourlyForecast({ lat: latitude, lon: longitude })
        );
  ...
  const handleCitySelect = async (city) => {
    ...
    dispatch(fetchHourlyForecast({ lat: city.lat, lon: city.lon }));
    dispatch(setSubscribedCity(city.name));
  ...
  const getDayAndDate = () => {
    ...
    return {
      day: localTime.toLocaleDateString("en-US", {
        weekday: "long"
      }),
      date: localTime.toLocaleDateString("en-GB")
    };
  };
        ...
        <div className="container-lg py-5">
          <section className="current-weather-section mb-5">
            ...
            <div className="alert alert-info d-flex align-items-center gap-2">
              <i className="bi bi-bell-fill"></i>
              Subscribed city:{" "}
              <strong>{subscribedCity || "None"}</strong>
            </div>
            ...
            {!loading && current && (
              <div className="row g-4">
                <div className="col-lg-6 col-md-12">
                  <div className="weather-card-large d-flex justify-content-between align-items-start">
                    <div>
                      <h3 className="weather-location-name">
                        {current.name}
                      </h3>
              ...
              <div className="forecast-card overflow-auto">
                <div className="d-flex gap-3 min-w-max">
                  {hourly.slice(0, 12).map((hour, idx) => (
                    <div
                      key={idx}
                      className="text-center forecast-hour"
                    >
                      <div className="weather-icon-hour">
                          <WeatherIcon icon={hour.weather[0].icon} size={32} />
                      </div>
