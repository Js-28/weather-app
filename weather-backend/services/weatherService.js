...
async function getHourlyForecast({ city, lat, lon, hours = 1 }) {
  ...
  try {
    ...
    console.log('Fetching hourly forecast:', { city, lat, lon, hours });
console.log('Coordinates used:', coordinates);
console.log('OpenWeather URL:', url);

    const response = await axios.get(url);

    console.log('OpenWeather response keys:', Object.keys(response.data));
console.log('Hourly data length:', Array.isArray(response.data.hourly) ? response.data.hourly.length : 0);
